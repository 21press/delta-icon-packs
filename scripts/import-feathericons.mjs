#!/usr/bin/env node
/**
 * Import temp/feathericons into packs/feather (single outline set).
 * Strips HTML comments; rewrites hard-coded fill/stroke → currentColor.
 *
 * Usage: node scripts/import-feathericons.mjs
 */
...
const srcRoot = path.join( root, 'temp', 'feathericons' );

const VARIANT = {
	slug: 'feather',
	name: 'Feather',
	description: '24×24 outline icons from Feather Icons.',
	src: srcRoot,
};

const HOMEPAGE = 'https://feathericons.com';
const VERSION = '1.0.0';
const LICENSE = 'MIT';

const PACK_README = `# Feather

Icons from [Feather Icons](https://feathericons.com).

- License: MIT (see upstream Feather license)
- Pack version: ${ VERSION }

Used with Delta Icon Manager as \`{{icon:feather/icon-name}}\`.
`;
...
if ( ! fs.existsSync( srcRoot ) ) {
	console.error( 'Missing temp/feathericons — place upstream SVGs there first.' );
	process.exit( 1 );
}
